		...
		if (!observable) {
			...
			const registerRead = (record, key, protoParents) => {
				...
				function initParentData () {
					...
					if (parentData == null) {
						parentData = {protoParents: _protoParents, protoPos: new Set()};
						parentData.protoPos.add(0);
						updates.parentData.set(record, parentData);
					}
					else if (parentData.protoParents.length < _protoParents.length) {
						parentData.protoParents = _protoParents;
					}
					if (isRoot) {
						parentData.protoPos.add(_protoParents.length - 1);
					}
				...
				while (proto != null) {
					...
					_protoParents.unshift(proto);
					if (observableProto != null) {
						if (!initParentData.initialized) {
							initParentData();
						}
						console.log("is observable proto");
						_protoParents = observableProto[manager.$registerRead](record, key, [..._protoParents]);

						return _protoParents;
					}
						...
						if (updates) {
							if (updates.parentData) {
								updates.records.forEach((record) => {
									...
									let invalidate = true;
									let l = parentData.protoParents.length;
									let d = parentData.protoParents.indexOf(obj);
									for (let i = d; i < l; i++) {
										console.log("go go", i, l, d);

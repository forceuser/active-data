...
import {Manager} from "active-data";
import {observable, computed, reaction, updatable} from "active-data";
...
test("Create reaction function (no-unregister)", t => {
	...
	t.equal([...m.reactionsToUpdate.values()].length, 0);
	m.makeReaction(reaction);
	t.equal([...m.reactionsToUpdate.values()].length, 1);
...
test("Observable prototype - overriden property - reaction", t => {
	...
	const reaction1 = sinon.spy(
		() => `${observables.A3.a} ${observables.A9.a} ${observables.A9.a}`
	);
...
test("Deep watch", t => {
	const {reaction, observable} = new Manager({
		immediateReaction: true,
		prototypes: true,
	});
	const srcProto = {
